...
export class ApiSearchService {
	...
	private apikey = '?api_key=ebbcc2bbea6a3127c6715e6d4e044f66';

	constructor(private http: Http) { }

	// Get to ten
	getTopTen() {
		return this.http.get(`${this.base_url}/tv/popular${this.apikey}&language=en-US&page=1&`).map((res: Response) => res.json());
	}
	...
	*/
	...
	findFinalEpisode(itemid, seasonNumber) {
		const data = this.http.get(`${this.base_url}/tv/${itemid}/season/${seasonNumber}${this.apikey}&language=en-US&page=1&`).map(res => res.json())
		return data;
	}
	...
	getGenres(idsArray) {
		...
		for (let i = 0; i < idsArray.length; i++) {
			const data = this.http.get(`${this.base_url}/tv/${idsArray[i]}${this.apikey}&language=en-US&page=1&`).map(res => res.json());
